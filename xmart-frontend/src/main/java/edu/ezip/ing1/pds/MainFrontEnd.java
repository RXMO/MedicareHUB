 ...
 * } catch (Exception e) {
 * logger.error("Erreur lors de la personnalisation de l'interface : {}",
 * e.getMessage());
 * }
 * 
 ...
 * }
 * 
 * private static void createUI(Patients patients, PatientService
 * patientService) {
 ...
 * });
 * 
 * // ActionListener pour le bouton Médecins
 ...
 * } catch (Exception ex) {
 * JOptionPane.showMessageDialog(null,
 * "Erreur lors de l'ouverture de l'interface médecin : " + ex.getMessage());
 * ex.printStackTrace();
 * }
 * });
 * 
 * // ActionListener pour le bouton Omar (Patients)
