...
public class LoggingUtils {
    ...
    public static void logDataMultiLine(final Logger logger, Level level, byte [] data) {
        logDataMultiLine(logger, level, new String(data));
    }
    public static void logDataMultiLine(final Logger logger, Level level, String data) {
        try {
            initFunctions();
            final String [] bylines = data.split("\n");
